...
# Setup Logging (stabile Version ohne Absturzpotential)
logging.basicConfig(
...
def get_shelly_devices():
    ...
    try:
        ...
        logging.error(f"Fehler beim Lesen der Gerätedatei ({DEVICES_FILE}): {e}")
        return []
...
def setup_scan():
    ...
    try:
        ...
        def on_service_added(zeroconf_instance, type, name):
            ...
            try:
                ...
                if info and b'shelly' in info.properties.get(b'model', b''):
                    ...
                    if not any(d['ip'] == ip_address for d in found_devices):
        ...
        if not found_devices:
            return jsonify({"status": "error", "message": "Keine Shelly-Geräte gefunden."})
...
def configure_start():
    ...
    if not personal_ssid: # Passwort kann leer sein
        return jsonify({"status": "error", "message": "WLAN-SSID fehlt."})

    devices = get_shelly_devices()
    if not devices:
        return jsonify([{"type": "warning", "message": "Keine Geräte zum Konfigurieren gespeichert."}])
    ...
    for device in devices:
        ...
        ip = device.get('ip')
        if not ip:
            current_status.append({"type": "error", "message": f"Gerät '{ssid}' hat keine IP. Überspringe..."})
            continue
        try:
            current_status.append({"type": "progress", "message": f"Konfiguriere '{ssid}'..."})
            url = f"http://{ip}/settings/sta?ssid={encoded_ssid}&password={encoded_password}&enable=true"
            response = requests.get(url, timeout=15)
            if response.status_code == 200:
                current_status.append({"type": "success", "message": f"'{ssid}' erfolgreich konfiguriert."})
            else:
                current_status.append({"type": "error", "message": f"'{ssid}' API-Fehler: HTTP {response.status_code}"})
        except requests.exceptions.RequestException as e:
            error_message = f"Fehler bei der Konfiguration von '{ssid}': {e}"
            current_status.append({"type": "error", "message": error_message})
